...
kubectl create secret tls nginx-console-tls --key tls-console.key --cert tls-console.crt -n minio-operator
...
#kubectl create secret tls nginx-tls --key tls-minio.key --cert tls-minio.crt -n minio-operator
